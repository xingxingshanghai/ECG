...
public class StaticReceive {
	 ...
	 * @param bluName
	 */
	public static void startReceive(Context context, String bluName, BLUReader iReader, BLUSender iSender, Handler _handler) {
	...
	public static List<Double> datas = new ArrayList<Double>();
	...
	private static class ECGCallBack implements IECGCallBack {
		...
		@Override
		public void OnGetRealTimeResult(String time, int nTransMode, int nResult, int arg3) {
			//DrawThreadPC80B.pause = true;
		...
		@Override
		public void OnGetRealTimeMeasure(boolean arg0, ECGData arg1, int arg2, int arg3, int arg4, int arg5) {
			double temp[] = new double[arg1.data.size()];
			//datas.clear();
			for (int i = 0;i<arg1.data.size();i++) {
				temp[i] = (arg1.data.get(i).data/100.0-20);
				//datas.add(arg1.data.get(i).data/10.0);
			}
			Message msg = mHandler.obtainMessage();
			msg.what = MSG_DATA_ECG_STATUS_CH;
			msg.arg1 = 5;
			...
			data.putDoubleArray("datas",temp);
			msg.setData(data);
			mHandler.sendMessage(msg);
			DRAWDATA.addAll(arg1.data);
			for (int i = 0;i<arg1.data.size();i++) {
				DataWriteFile(String.valueOf(arg1.data.get(i).data));
			}


		}
		private void DataWriteFile(String stringHex) {
			File file = new File(Environment.getExternalStorageDirectory(), "20171121.txt");
			method2(file.getPath(),stringHex+"\n");
		}
		/**
		 * 追加文件：使用FileWriter
		 *
...
//				DataWriteFile(String.valueOf(arg1.data.get(i).data));
		...
		@Override
		public void OnGetRequest(String sDeviceID, String sProductID, int nSmoothingMode, int nTransMode) {
			mHandler.obtainMessage(MSG_DATA_ECG_STATUS_CH, 7, nSmoothingMode, nTransMode).sendToTarget();
		}
		...
		public void OnReceiveTimeOut() {
			mHandler.obtainMessage(MSG_DATA_ECG_STATUS_CH, MSG_DATA_TIMEOUT, 0, 0).sendToTarget();
		}
	}
