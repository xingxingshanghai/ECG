		...
		if (start) {
			try {
				if (MyBluetoooth.bluSocket != null) {
					String conDeviceName = MyBluetoooth.bluSocket.getRemoteDevice().getName();
					BLUReader reader = new BLUReader(MyBluetoooth.bluSocket.getInputStream());
					BLUSender sender = new BLUSender(MyBluetoooth.bluSocket.getOutputStream());
					StaticReceive.startReceive(this, conDeviceName, reader, sender, mHandler);
				}
			} catch (IOException e) {
				e.printStackTrace();
			}
